...
execute at @p[scores={TC_AWP_Ammo=1..10},distance=..1] positioned ~ ~1.6 ~ run summon arrow ^ ^ ^0.1 {CustomName:'{"text":"AWP"}',Tags:["Bullet","New","Player"],PierceLevel:2,damage:3.8,SoundEvent:"minecraft:block.stone.break"}
execute at @p[scores={TC_AWP_Ammo=1..10},distance=..1] run summon armor_stand ~ ~1.6 ~ {Tags:["BulletAnchor","Player"],Invisible:1b,Small:1b,NoGravity:1b}
...
execute at @a[scores={TC_Recoil=36,TC_Shifting=1..},distance=..1] run tp @p ~ ~ ~ ~ ~-12.5
