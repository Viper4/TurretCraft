...
effect give @a[tag=TC_RemoteControl] fire_resistance 2 255 true
...
execute as @e[type=armor_stand,tag=!Firing,tag=TC,tag=TurretC,tag=TC_RemoteControl,limit=1,sort=nearest] if score @s TC_Frequency = @p[tag=TC_RemoteControl,scores={TC_RightClick=1..,TC_Shifting=0}] TC_Frequency run tag @s add Firing
